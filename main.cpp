...
int main()
{
    ...
    while (window.isOpen())
    {
        ...
        case JUGANDO: //lucas agregué un prefetch para corregir la cortina negra del juego, agregué codigo pero no cambie lo tuyo.
                ...
                // Dibujo un vecindario alrededor del chunk actual (radio 2 => 5x5 chunks).
                // llamo varias veces a draw2x2Framed.
                {
                    const sf::Vector2i grid = player.getPosGrilla(); //tome el centro del mundo segun el chunk actual donde esta el pj

                    // Primero el chunk actual (mantengo tu orden)
                    ...
                    const sf::Vector2f centerG = { static_cast<float>(grid.x) * Map::CELL, static_cast<float>(grid.y) * Map::CELL }; //multiplico por map cell para conseguir la posicion en el mapa donde va caer la proxima interseccion.
                    ...
                    // dibujo también un anillo adicional (radio 2) para que nunca falte el frente.
                    for (int dy = -2; dy <= 2; ++dy) {
                        for (int dx = -2; dx <= 2; ++dx) {
                            if (dx == 0 && dy == 0) continue; // ya dibujado
                            map.draw2x2Framed(window, grid + sf::Vector2i(dx, dy));
                        }
                    }
                }
